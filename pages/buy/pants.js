...
export default function PantsCustomizer() {
  ...
  renderer.setClearColor(0x000000, 0)
    ...
    // Load sweatpants model
    const loader = new GLTFLoader()
    loader.load('/models/sweatpants.gltf', (gltf) => {
      const pantsModel = gltf.scene
      meshRef.current = pantsModel
      
      const box = new THREE.Box3().setFromObject(pantsModel)
      const size = box.getSize(new THREE.Vector3())
      const maxDim = Math.max(size.x, size.y, size.z)
      const scale = 2 / maxDim
      pantsModel.scale.multiplyScalar(scale)
      
      box.setFromObject(pantsModel)
      const center = box.getCenter(new THREE.Vector3())
      pantsModel.position.set(-center.x, -center.y, -center.z)
  ...
  const updateTexture = () => {
    ...
    // Convert grey pants colors to white
    let fillColor = pantsColor
    // Parse hex color to RGB
    const hex = pantsColor.replace('#', '')
    ...
    // Clear canvas with pants color background
  ...
  useEffect(() => {
    const handleLoadDesign = (e) => {
      ...
      if (design.type !== 'pants') return
      
      // Load the design data
      setPantsColor(design.colorHex || '#ffffff')
    ...
    if (framesJson) {
      try {
        ...
        if (frames && frames.length > 0) {
          ...
          console.log('Auto-loading generated frame for pants:', frameUrl)
          
          // Create an image element to get dimensions
          const img = new Image()
          img.crossOrigin = 'anonymous'
          img.onload = () => {
            // Convert pixel coordinates (396, 598) to normalized UV coordinates (0-1)
            ...
            console.log('Added generated frame as overlay for pants:', newOverlay)
          }
          img.onerror = () => {
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      type: 'pants',
      price: 45.00,
      color: colorNames[pantsColor] || 'Custom',
      colorHex: pantsColor,
    ...
    <main style={{ 
      ...
      <div style={{ 
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '12px', padding: '12px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: '#666', fontSize: '0.9rem' }}>Custom Pants · {pantsColor}</div>
                </div>
          ...
          <div style={{
            ...
            <div 
              ...
              style={{
                ...
                aspectRatio: '1',
                cursor: 'crosshair',
                background: `url(/uv-maps/pants-uv.png)`,
            ...
            <div style={{
              ...
              Click and drag to position on pants
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          {itemSelectedMode ? 'Item selected - Click away to rotate pants' : '← Click and drag to rotate →'}
        </div>
      </div>
      ...
      <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              <button
                ...
                onClick={() => setPantsColor(colorOption.color)}
                style={{
                  width: '100%',
                  aspectRatio: '1',
                  background: colorOption.color,
                  border: `3px solid ${pantsColor === colorOption.color ? '#000' : '#ddd'}`,
                  borderRadius: '8px',
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                  boxShadow: pantsColor === colorOption.color ? '0 0 0 2px #4db8ff' : 'none',
                }}
        ...
        >
          {overlays.length === 0 ? 'Add designs to continue' : 'Add to Cart - $45.00'}
        </button>
      </div>
