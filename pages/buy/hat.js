...
export default function ShirtCustomizer() {
  ...
  renderer.setClearColor(0x000000, 0)
    ...
    loader.load('/models/hat.gltf', (gltf) => {
      const hatModel = gltf.scene
      meshRef.current = hatModel
      
      const box = new THREE.Box3().setFromObject(hatModel)
      const size = box.getSize(new THREE.Vector3())
      const maxDim = Math.max(size.x, size.y, size.z)
      const scale = 2 / maxDim
      hatModel.scale.multiplyScalar(scale)
      
      box.setFromObject(hatModel)
      const center = box.getCenter(new THREE.Vector3())
      hatModel.position.set(-center.x, -center.y, -center.z)
  ...
  const updateTexture = () => {
    ...
    // Convert grey hat colors to white
    ...
    // Clear canvas with hat color background
    ...
    if (framesJson) {
      try {
        ...
        if (frames && frames.length > 0) {
          ...
          img.onload = () => {
            // Convert pixel coordinates (363, 712) to normalized UV coordinates (0-1)
    ...
    <main style={{ 
      ...
      <div style={{ 
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '12px', padding: '12px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: '#666', fontSize: '0.9rem' }}>Custom Hat · {shirtColor}</div>
                </div>
          ...
          <div style={{
            ...
            <div 
              ...
              style={{
                ...
                aspectRatio: '1',
                cursor: 'crosshair',
                background: `url(/uv-maps/hat-uv.png)`,
            ...
            <div style={{
              ...
              Click and drag to position on hat
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          {itemSelectedMode ? 'Item selected - Click away to rotate hat' : '← Click and drag to rotate →'}
        </div>
      </div>
      ...
      <div style={{
        ...
        >
          {overlays.length === 0 ? 'Add designs to continue' : 'Add to Cart - $25.00'}
        </button>
      </div>
